...
from agavepy.actors import get_context, send_python_result
...
if __name__ == '__main__':
    ...
    context = get_context()
    try:
        cores = int(context['message_dict']['cores'])
        n_bits = int(context['message_dict']['bits'])
        n_blocks = int(context['message_dict']['blocks'])
    except Exception:
    ...
    logs['messages'] += f' Mining {n_bits}-bit blocks out to the {n_blocks}th block on {cores} core(s)..'
    ...
    logs['messages'] += verifyChain(chain)

    # send hashrate to results endpoint
    #send_python_result(hashrate)
