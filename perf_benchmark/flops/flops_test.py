...
def flop_tester(actor_id, nodes, runs, workers_per_node, messages_per_worker, threads, std_dev, size)
...
# Create workers for actor in batches of 'incr' to insure it works.
...
def main():
    ...
    # First set of runs, six workers per node, easy work.
    # Create Actor
    ...
    # Second set of runs, 1 worker per node, hard work.
    # Create Actor
    actor_id = create_actor()
    # Create Workers
    create_workers(actor_id, num_nodes)
